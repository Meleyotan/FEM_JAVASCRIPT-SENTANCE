...
//When you think you completed createFunction, un - comment out those lines in the code and run it
//to see if it works.
...
//return that output.If it is called any additional times, instead of calling the callback
...
//before allowing the callback to be invoked as the second parameter.Any additional arguments
//after wait are provided to func when it is invoked.HINT: research setTimeout();
...
//Also, the function being passed in will accept at least one argument.defineFirstArg will
//return a new function that invokes the passed -in function with the passed -in argument as
//the passed -in function's first argument. Additional arguments needed by the passed-in function
...
//and an output key that contains the result from invoking the passed -in function.HINT:
...
//the returned function will return the same string, except all instances of first strings(of saved pairs)
...
//an object with ONLY two methods.getSecret() which returns the secret setSecret() which sets the secret
...
// Challenge 14
// Write a function, callTimes, that returns a new function. The new function should return
...
//The returned function will take no arguments, and will return the string 'spin'
//the first n - 1 number of times it is invoked.On the very next invocation(the nth invocation),
//the returned function will return the string 'win'.On every invocation after that,
//the returned function returns the string 'pick a number to play again'.
...
//of all the numbers have ever been passed into that function
//(duplicate numbers count just like any other number). When the returned function is invoked
//with no arguments, the current average is outputted.If the returned function is invoked with
...
//and returns a function (that will accept a callback). The returned function should return
//true if the first elements(of each sub - array) being passed into the callback all yield the
//corresponding second elements(of the same sub - array).Otherwise, the returned function should
//return false.
...
// Challenge 18
// Create a function makeHistory that accepts a number (which will serve as a limit), and returns a
//function(that will accept a string).The returned function will save a history of the most recent
//"limit" number of strings passed into the returned function (one per invocation only). Every time a
//string is passed into the function, the function should return that same string with the word 'done'
//after it(separated by a space).However, if the string 'undo' is passed into the function, then the
//function should delete the last action saved in the history, and return that deleted string with the word
//'undone' after(separated by a space).If 'undo' is passed into the function and the function's history is
...
// Create a function blackjack that accepts an array
//(which will contain numbers ranging from 1 through 11), and returns a DEALER function.
//The DEALER function will take two arguments(both numbers), and then return yet ANOTHER
//function, which we will call the PLAYER function.

// On the FIRST invocation of the PLAYER function, it will return the sum of the two numbers
//passed into the DEALER function.

// On the SECOND invocation of the PLAYER function, it will return either:

// the first number in the array that was passed into blackjack PLUS the sum of the two numbers
//passed in as arguments into the DEALER function, IF that sum is 21 or below, OR
...
// And again, if it is 'bust', then every subsequent invocation of the PLAYER function will return
//the string 'you are done!'.Otherwise, it can continue on to give the next sum with the next number
